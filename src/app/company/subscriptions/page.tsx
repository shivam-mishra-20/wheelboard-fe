...
const SubscriptionsPage = () => {
  ...
  const computeDisplay = (priceValue: number) => {
    const yearly = priceValue;
    const monthlyFromYearly = Math.round(yearly / 12);
    if (billingCycle === 'yearly') {
      return {
        display: `₹${yearly.toLocaleString()}`,
        period: '/year',
      };
    }
    return {
      display: `₹${monthlyFromYearly.toLocaleString()}`,
      period: '/month',
    };
  };
      ...
      <div className="mx-auto max-w-7xl">
        ...
        >
          ...
          <p className="mx-auto max-w-2xl text-lg text-gray-600">
            Choose the perfect plan for your company needs. Upgrade, downgrade,
            or cancel anytime.
          </p>
        </motion.div>
          ...
          <div className="rounded-full bg-white p-1 shadow-sm">
            <div className="flex items-center gap-1 rounded-full bg-gray-100 p-1">
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${billingCycle === 'monthly' ? 'bg-white text-primary-600 shadow' : 'text-gray-600'}`}
              >
                Monthly
              </button>
              <button
                onClick={() => setBillingCycle('yearly')}
                className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${billingCycle === 'yearly' ? 'bg-primary-500 text-white shadow' : 'text-gray-600'}`}
              >
                Yearly
              </button>
            </div>
          ...
          {plans.map((plan, index) => (
            ...
            >
              ...
              <div className="p-8">
                ...
                <div className="mb-2">
                  {(() => {
                    const p = computeDisplay(plan.priceValue);
                    return (
                      <>
        ...
        >
          ...
          <p className="mb-6 text-primary-100">
            Our team is here to help you choose the right plan for your company
          </p>
